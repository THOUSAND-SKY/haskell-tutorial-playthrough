...
import Blogger.Html.Html (html_, p_, render, txt_, ul_)
import Test.Hspec
...
spec = do
  describe "html" $ do
    it "produces head with title + body" $ do
      render (html_ "My page title" (p_ $ txt_ "My page content")) `shouldBe` "<html><head><title>My page title</title></head><body><p>My page content</p></body></html>"
